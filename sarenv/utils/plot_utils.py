...
import numpy as np # For cv2 related array manipulation if re-enabled

# from ..utils.logging_setup import get_logger # If logging is needed here
# log = get_logger()


def plot_basemap(ax=None, source=ctx.providers.Esri.WorldImagery, crs="EPSG:4326"): # Default to WGS84 for basemaps typically
    """
    ...
    try:
        ...
        print(f"Could not add basemap: {e}. Ensure you have an internet connection and an appropriate CRS.")
    return ax


def normalize_coordinates(boundary_polygon: shapely.Polygon, geometries_to_normalize: list | shapely.MultiPolygon | shapely.MultiLineString):
    """
    ...
    elif hasattr(geometries_to_normalize, 'geoms'): # Handles MultiPolygon, MultiLineString
        geoms_to_process = list(geometries_to_normalize.geoms)
    elif isinstance(geometries_to_normalize, shapely.geometry.base.BaseGeometry): # Single geometry
        geoms_to_process = [geometries_to_normalize]
