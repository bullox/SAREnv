...
class ComparativeEvaluator:
    ...
    """
    def __init__(self, dataset_directory="sarenv_dataset", evaluation_sizes=None, num_drones=3, num_victims=100):
        """
        ...
        self.evaluation_sizes = evaluation_sizes or ["small", "medium", "large", "xlarge"]
    ...
    def load_datasets(self):
        ...
        for size in self.evaluation_sizes:
            ...
            victims_gdf = gpd.GeoDataFrame(geometry=victim_points, crs=data_crs) if victim_points else pd.GeoDataFrame(columns=['geometry'], crs=data_crs)
            
            self.environments[size] = {'item': item, 'victims': victims_gdf, 'crs': data_crs}
        log.info("All datasets loaded and prepared.")

    def run_baseline_evaluation(self) -> pd.DataFrame:
        """
        ...
        for size, env_data in self.environments.items():
            ...
            evaluator = metrics.PathEvaluator(item.heatmap, item.bounds, victims_gdf, self.path_params['fov_deg'], self.path_params['altitude'])
            ...
            for name, generator in self.baseline_generators.items():
                ...
                result = {
                    ...
                    "Time-Discounted Score": discounted,
                    "Victims Found (%)": victim_score['percentage_found'],
                    "Detection Timeliness": victim_score['detection_timeliness']
                }
    ...
    def plot_results(self, results_df: pd.DataFrame = None, output_dir="graphs"):
        ...
        for metric in metrics_to_plot:
            ...
            sns.barplot(data=results_df, x="Dataset", y=metric, hue="Algorithm", order=self.evaluation_sizes)
            ...
            plot_filename = os.path.join(output_dir, f"plot_{metric.replace(' ', '_').lower()}.png")
...
if __name__ == '__main__':
    ...
    # You can customize which datasets to run on, e.g., evaluation_sizes=["small", "large"]
    evaluator = ComparativeEvaluator()

    # 2. Load the datasets (one line)
    evaluator.load_datasets()

    # 3. Run all baselines on all loaded datasets (one line)
    baseline_results = evaluator.run_baseline_evaluation()

    # 4. Plot the results from the baseline run
