...
class GeospatialDataLoader:
    ...
    def download_geojson(self, boundary_points, output_file="downloaded_features.geojson"):
        """
        ...
        for feature_type, gdf in features_by_type.items():
            if gdf is not None and not gdf.empty:
                ...
                if not polygon_geoms.empty:
                    ...
                    multi_poly = gpd.GeoDataFrame(geometry=[polygon_geoms.unary_union], crs=gdf.crs)
    ...
    def load_geojson(self):
        ...
        boundary_feature = [f for f in self.data['features'] if f.get('id') == 'boundary'][0]
        self.boundary = gpd.GeoDataFrame.from_features([boundary_feature], crs="EPSG:4326")
        nested_collection = [f for f in self.data['features'] if f.get('type') == 'FeatureCollection'][0]

        # Extract features from nested collection; the GeoJSON coordinates are (lon, lat)
        feature_types = ['structure', 'road', 'linear', 'drainage', 'water',
                         'brush', 'scrub', 'woodland', 'field', 'rock']

        for feature_type in feature_types:
            features = [f for f in nested_collection['features'] if f.get('id') == feature_type]
            if features:
        ...
        loaded_features = {ft: len(getattr(self, ft)) if getattr(self, ft) is not None else 0
                           for ft in feature_types}
        print(f"Loaded: Boundary ({len(self.boundary)}), " +
    ...
    def visualise_geo_data(self):
        ...
        for feature_type in feature_types:
            gdf = getattr(self, feature_type)
            if gdf is not None and not gdf.empty:
                legend_handles.append(Patch(facecolor=color_mapping[feature_type], edgecolor='black', label=feature_type.capitalize()))
        if legend_handles:
